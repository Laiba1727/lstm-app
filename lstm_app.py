...
if uploaded_file is not None:
    ...
    st.write(f"Shape of train_sentences: {train_sentences.shape}")
    st.write(f"Shape of train_labels: {train_labels.shape}")
    st.write(f"Shape of test_sentences: {test_sentences.shape}")
    st.write(f"Shape of test_labels: {test_labels.shape}")

    st.write(f"Check for NaNs in train_sentences: {np.isnan(train_sentences).sum()}")
    st.write(f"Check for NaNs in train_labels: {np.isnan(train_labels).sum()}")
